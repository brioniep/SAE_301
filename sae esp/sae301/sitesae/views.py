...
def get_temperature(request):
    ...
    if is_within_schedule(start_time, end_time):
        ...
        # En dehors de la plage, renvoyer une réponse vide ou un message d'erreur
    ...
    while True:
        ...
        for schedule in schedules:
            if schedule.start_time <= current_time <= schedule.end_time:
                # Si l'heure actuelle est dans la plage, allumer la prise
                mqtt_client.envoi(f"turn_on_{schedule.prise_id}")
            else:
                # Si l'heure actuelle est en dehors de la plage, éteindre la prise
...
def enforce_schedules(request):
    check_schedules()
    return JsonResponse({'status': 'Schedules enforced'})
